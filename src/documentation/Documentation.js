...
const Documentation = () =>
    <div className='documentation-container'>
      ...
      <h1 className='electro-font center-this-div'>Documentation</h1>
              <div id='overview'> <h2  className='title-doc' >Overview </h2>
              <p>
                Brainfreeze takes advantage of redux architecture. That means that a lot of how Brainfreeze works
                is very similar to how redux, or a redux-like library works. The basic structure of Brainfreeze follows the
                dispatch an action => reducer => update/change state => return a new state.
              </p>
              <p>
                Brainfreeze still follows the functional approach that redux follows, we feel that is an appropriate approach to
                state containers.
              </p>
              <p><strong>Where does Brainfreeze differentiate itself?</strong></p>
              <p>Brainfreeze saw an issue where as applications scaled, the amount of 'listeners' on the page that were reacting to
              a state change could become taxing on performence. Brainfreeze focuses on the developer gaining more control over the
              their application state. We allow for this by creating a flattened state model that is able to create a mapping of the state
               object. By doing this we allow specific pieces of state to map to their specific listener. The listener now will only react
             to the changes that are neccessary to them when state is set. </p>
             <p>
               Doing this, we also enabled middleware functionality for more control over the state object. By creating this model, we set out
               create less business logic needed in the reducers and create more room for the developer to code what they want to code, and let the library
               hash out the unwanted logic. We are excited to dive into the depths of what Brainfreeze can do.
             </p>
         </div>
            <div><h3 id='api-reference' className='title-doc' >API REFERENCE</h3>
                <blockquote> Brainfreeze aims to provide a familiar and predictable state container.</blockquote>
            </div>
          <ul>
            <li className='no-bullet docs-code' id='installation'><blockquote> Installation
